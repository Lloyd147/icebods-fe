import { getBestBookie } from '@/lib/utils';
...
function updateItems(state: BetSlipState, payload: BetslipItem, actionType: string) {
  ...
  state.selectedBookie = state.items.length > 0 ? getBestBookie(state.totalOddsPerBookie) : '';
  ...
  items.forEach((item) => {
    ...
    for (const bookie in item.odds) {
      if (item.odds.hasOwnProperty(bookie)) {
        const odds = item.odds[bookie].oneX[selectedType as keyof OneX];
        if (odds === 0) continue;
